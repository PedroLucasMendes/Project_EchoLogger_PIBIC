...
class Controller:
    state="STOP" #"RECORD":0,"PAUSE":1,"RESUME":2,"STOP":3 
    base_path=None
    file_path=None
    file_length=0
    record_timer=Timer(0)

    def __init__(self, sck=32, ws=25, sd=33, i2s_id=0, buffer_length=80000, sample_rate=22050, sample_bits=16, volume=0):
        self.channels = 1        
        self.sample_size = sample_bits // 8  # Converte bits por amostra para bytes
        self.sample_rate = sample_rate
        self.sample_bits = sample_bits
        self.chunk_size = self.calculate_chunk_size()  # Calcula chunk size para 60 segundos
        self.file_counter = 0
        self.volume_mic = volume
        self.audio_in = I2S(
            i2s_id,
            sck=Pin(sck),
            ws=Pin(ws),
    ...
    def calculate_chunk_size(self):
        """Calcula o tamanho do chunk para 60 segundos de gravação."""
        return self.sample_rate * self.sample_size * self.channels * 60
    ...
    def i2s_callback_rx(self, args):
        if self.state == "RECORD":
            if self.file_length >= self.chunk_size:
                ...
                audio_samples = self.mic_samples_mv[:self.file_length]
                
                I2S.shift(buf=audio_samples, bits=self.sample_bits, shift=self.volume_mic)  
                
                num_bytes_written = self.file_path.write(audio_samples)
