...
if(cond == True):
     ...
     if config:
          ...
          _SLEEP_TIME = config.get("sleep cicle", 5) * 60  # Tempo total em minutos convertido para segundos
          
          _READINGS_INTERVAL = _SLEEP_TIME  # Tempo de sleep
          _SAMPLES_LOOPS = _AUDIO_LENGTH // _READINGS_INTERVAL
     else:
          _AUDIO_LENGTH = 300  # Padrão de 5 minutos se o arquivo config não for lido
          _READINGS_INTERVAL = 900  # Padrão de 15 minutos de sleep
          _SAMPLES_LOOPS = _AUDIO_LENGTH // _READINGS_INTERVAL
